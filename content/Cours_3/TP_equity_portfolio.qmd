...
- If you finish the exercise early, you can implement method to populate the attribute of the Portfolio class that
  are not handle yet.
...
class Position:
    def __init__(self, instrument: Instrument, date: datetime, weight: float = 0, quantity: float = 0):
...
1. **Class Attributes**:
   Based on the provided code framework, create a `Portfolio` class which should have the following attributes:
...
6. **Portfolio Summary**:
   ...
   portfolio's current positions, including the ticker symbols, weights, quantities, and last close for the assets
